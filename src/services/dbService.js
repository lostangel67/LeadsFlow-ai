...
async function saveConfig(userId, config) {
  ...
  if (error) {
    ...
    throw new Error("Erro ao salvar configurações. Verifique sua conexão e tente novamente.");
  }
  return true;
}
...
async function criarSessao(userId) {
  ...
  if (error) {
    ...
    throw new Error("Erro ao criar conversa. Verifique sua conexão e tente novamente.");
  }
