...
function createMapsView() {
  ...
  // NOT added to mainWindow — runs invisible in background
...
function showMapsView() {
  ...
  // Never add to mainWindow — stays invisible
}

function hideMapsView() {
  // Maps view is never added to window — nothing to remove
}

// ===================== MAPS WORKER POOL =====================
const POOL_SIZE = 6;
let mapsPool = [];
let mapsPoolReady = false;

function initMapsPool() {
  if (mapsPoolReady) return;
  mapsPool = [];
...
function destroyMapsPool() {
  for (const w of mapsPool) {
...
function createWindow() {
  ...
  mainWindow.on("resize", () => {
    positionEmbeddedView(whatsappView);
    // Maps view is never added to window — no repositioning needed
  });
...
ipcMain.handle("ai-sugerir-nichos", async (_, params) => {
  ...
  try {
    const { produto, score, objetivo, cidade } = typeof params === "object" ? params : { produto: params, score: 40 };
    const nichos = await sugerirNichos(produto || "", objetivo || "", cidade || "");
...
ipcMain.handle("ai-auto-config", async (_, { nicho, produto, objetivo, cidade }) => {
  try {
    if (!produto) return { error: "Descreva o produto antes de configurar" };
    const resultado = await autoConfigurar(nicho, produto, objetivo || "", cidade || "");

    // Auto-save: merge AI result into existing config
    const current = await loadConfig();
    const updated = {
      ...current,
      nicho: resultado.nichos_sugeridos?.[0] || current.nicho,
      nichos: resultado.nichos_sugeridos || current.nichos,
      nichos_texto: (resultado.nichos_sugeridos || []).join(", "),
      modo: resultado.cidade_recomendada === "brasil" ? "brasil" : "cidade",
      cidade: resultado.cidade_recomendada === "brasil" ? "" : (resultado.cidade_recomendada || current.cidade),
      delay_min: resultado.delay_min || current.delay_min,
      delay_max: resultado.delay_max || current.delay_max,
      limite_diario: resultado.limite_diario || current.limite_diario,
      ai_tom: resultado.tom || current.ai_tom,
      ai_min_score: resultado.min_score || current.ai_min_score,
      schedule_enabled: true,
      schedule_start: resultado.horario_inicio || current.schedule_start,
      schedule_end: resultado.horario_fim || current.schedule_end,
      schedule_lunch_start: resultado.almoco_inicio || current.schedule_lunch_start,
      schedule_lunch_end: resultado.almoco_fim || current.schedule_lunch_end,
      schedule_days: resultado.dias_semana || current.schedule_days,
      max_per_hour: resultado.max_per_hour || current.max_per_hour,
    };
...
ipcMain.handle("chat-create-session", async () => {
  ...
  try { return await db.criarSessao(currentUserId); } catch (err) { return { error: friendlyError(err.message) }; }
});
...
async function buscarLeadsMapEmbutido(config) {
  ...
  try {
    for (let i = 0; i < cidades.length; i++) {
      ...
      if (i < cidades.length - 1 && !shouldStop) await sleep(1000 + Math.random() * 1000);
    }
  } catch (err) {
    erros++;
...
// Phone extraction JS (reused by single and batch lookups)
const PHONE_EXTRACT_JS = `
  (function(){
...
// Batch phone lookup: abre N páginas em paralelo via mini BrowserWindows
const PHONE_BATCH_SIZE = 6;
async function batchPhoneLookup(alvos, numerosJaUsados, cidade, nicho, leads, shouldStopFn) {
  const UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36";
  let phoneSeq = 0;

  for (let i = 0; i < alvos.length; i += PHONE_BATCH_SIZE) {
    if (shouldStopFn()) break;
    const batch = alvos.slice(i, i + PHONE_BATCH_SIZE);
    const windows = [];
    ...
    for (const item of batch) {
      ...
      const w = new BrowserWindow({
        ...
        webPreferences: {
          partition: `persist:phone-${Date.now()}-${phoneSeq}`,
          contextIsolation: true, nodeIntegration: false, sandbox: false,
        },
      });
      w.webContents.setUserAgent(UA);
      windows.push({ win: w, item });
    }

    const results = await Promise.all(windows.map(async ({ win, item }) => {
      const url = item.href.startsWith('http') ? item.href : `https://www.google.com${item.href}`;
      try {
        await win.webContents.loadURL(url);
        await sleep(1000);
        const tel = await win.webContents.executeJavaScript(PHONE_EXTRACT_JS).catch(() => null);
        return { nome: item.nome, tel };
      } catch (_) {
        return { nome: item.nome, tel: null };
      }
    }));
    ...
    for (const r of results) {
      if (r.tel) {
        const formatted = formatarTelefone(r.tel);
        if (formatted && !numerosJaUsados.has(formatted)) {
          leads.push({ nome: r.nome, telefone: formatted, cidade, nicho });
          numerosJaUsados.add(formatted);
          emitLog("sucesso", `   ✅ ${r.nome} → ${formatted}`);
        }
      }
    }
  }
...
async function extrairLeadsDaCidadeEmbutido(wc, termoBusca, cidade, nicho, opcoes) {
  ...
  // Scroll até o fim da lista (scroll + altura + check fim em 1 call)
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const result = await wc.executeJavaScript(`
      (function(){
        ...
        return{found:true,altura:f?-1:c.scrollHeight,fim:f};
      })()
    `).catch(() => ({ found: false, altura: 0, fim: false }));

    if (!result.found) break;
    if (result.altura === -1) break;

    await sleep(600);

    if (result.altura === alturaAnterior) {
      semMudancas++;
      if (semMudancas >= 3) break;
    } else {
      semMudancas = 0;
    }
    alturaAnterior = result.altura;
  }
  ...
  if (leads.length < limiteReal && semTel.length > 0 && !shouldStop) {
    ...
    // Batch phone lookup: 3 páginas em paralelo
    await batchPhoneLookup(alvos, numerosJaUsados, cidade, nicho, leads, () => shouldStop || leads.length >= limiteReal);
...
async function buscarLeadsMultiplosNichos(config, opts = {}) {
  ...
  emitWorkflow("init", "running", `${combinacoes.length} buscas (pool de ${POOL_SIZE} workers)`);
  emitLog("info", `🗂️ ${combinacoes.length} buscas com pool de ${POOL_SIZE} workers...`);
  showMapsView();
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("show-maps-panel");
  }

  // Worker pool: reuse fixed number of browser windows
  initMapsPool();
  emitWorkflow("init", "done", `Pool de ${POOL_SIZE} workers pronto`);

  const resultados = [];
  let idx = 0;

  async function runNext() {
    while (idx < combinacoes.length && !shouldStop) {
      const myIdx = idx++;
      const combo = combinacoes[myIdx];
      const termoBusca = `${combo.nicho} em ${combo.cidade}`;
      // Pick worker from pool (round-robin)
      const worker = mapsPool[myIdx % mapsPool.length];
      const wc = worker.webContents;

      try {
        emitLog("info", `🔍 [${myIdx + 1}/${combinacoes.length}] ${termoBusca}`);
        emitWorkflow("search", "running", `[${myIdx + 1}/${combinacoes.length}] ${termoBusca}`);
  ...
  // Run POOL_SIZE concurrent workers
  const workers = [];
  for (let i = 0; i < POOL_SIZE; i++) {
...
ipcMain.handle("start-autopilot", async () => {
  ...
  try {
    ...
    emitStatus({ state: "erro", message: `Erro: ${friendlyError(err.message)}` });
  } finally {
    isRunning = false;
    destroyMapsPool();
    hideMapsView();
...
function setupAutoUpdater() {
  ...
  autoUpdater.on("download-progress", (progress) => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("update-status", { status: "downloading", progress: Math.round(progress.percent) });
    }
  });
